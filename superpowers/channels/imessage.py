...
class IMessageChannel(Channel):
    ...
    def send(self, target: str, message: str) -> SendResult:
        if sys.platform != "darwin":
            return SendResult(
                ok=False, channel="imessage", target=target,
        ...
        script = (
            f'tell application "Messages" to send "{escaped_msg}"'
            f' to buddy "{escaped_target}"'
        )
        ...
        try:
            ...
            return SendResult(
                ok=True, channel="imessage", target=target,
                message="sent",
            )
    ...
    def test_connection(self) -> SendResult:
        if sys.platform != "darwin":
            return SendResult(
                ok=False, channel="imessage", target="",
                error="iMessage requires macOS",
            )

        script = (
            'tell application "System Events" to '
            '(name of processes) contains "Messages"'
        )
        ...
        try:
            ...
            running = output == "true"
            return SendResult(
                ok=True, channel="imessage", target="",
