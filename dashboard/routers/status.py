...
def aggregate_status():
    ...
    try:
        ...
        ga = get_github_admin()
        authed, _ = ga.is_authenticated()
        subs.append(
            SubsystemStatus(
                name="github", ok=authed, detail="authenticated" if authed else "not authenticated"
            )
        )
    except (ImportError, OSError, RuntimeError):
        subs.append(SubsystemStatus(name="github", ok=False, detail="unavailable"))
    ...
    except (ImportError, httpx.HTTPError, OSError, ValueError, KeyError):
